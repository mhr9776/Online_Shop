...
class AddProductUseCase @Inject()(productCallback: ProductCallback, userCallback: UserCallback, permissionCallback: PermissionCallback, sessionCallback: SessionCallback) extends AddProductService {
  ...
  override def call(body: AddProductService.Body)(implicit ec: ExecutionContext): Future[Product] = for {
    ...
    permissionOption <- permissionCallback get(body.userId, UserPermission.Permission.Add_Product)
    ...
    _ = productOption getOrElse{(new Exception(s"Product with name '${body.name}' already exists"))
    }
