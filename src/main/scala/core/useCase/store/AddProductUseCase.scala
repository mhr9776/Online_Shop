...
class AddProductUseCase @Inject()(productCallback: ProductCallback, userPermissionCallback: UserPermissionCallback) extends AddProductService {
  ...
  } yield product getOrElse(throw new Exception("not"))
    }
